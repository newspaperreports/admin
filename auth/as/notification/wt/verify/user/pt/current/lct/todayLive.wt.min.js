...
const jsonUrl = "https://raw.githubusercontent.com/newspaperreports/admin/main/auth/as/notification/wt/verify/user/pt/current/lct/todayLive.wt.json";

// 🔗 HTML Layout URL (popup structure stored in index.html)
const htmlUrl = "https://cdn.jsdelivr.net/gh/newspaperreports/admin@main/auth/as/notification/wt/verify/user/pt/current/lct/todayLive.wt.css";

// 🔗 CSS File URL (your design file)
const cssUrl = "https://cdn.jsdelivr.net/gh/newspaperreports/admin@latest/auth/as/notification/wt/verify/user/pt/current/lct/todayLive.wt.css";
...
async function loadPopupHTML(retry = 0) {
  try {
    ...
    // ✅ এখানে আমরা HTML ইনজেক্ট করার আগে চেক করি পুরোনোটা আছে কিনা
    if (!document.getElementById("adminNotification")) {
      ...
      console.log("ℹ️ Popup HTML already present.");
    }

    // 🔄 ইনজেকশন সফল হয়েছে কিনা চেক করা (DOM-এ আছে কিনা)
    if (!document.getElementById("adminNotification") && retry < 3) {
      // আবার চেষ্টা করুন (ফেচ লেট হলে বা DOM রেডি না হলে)
      setTimeout(() => loadPopupHTML(retry + 1), 500);
    } else if (!document.getElementById("adminNotification")) {
      console.error("❌ Popup HTML inject failed! #adminNotification not found after retries.");
    }
  } catch (error) {
...
function injectPopupCSS() {
  // আগের CSS থেকে ডাবল লোড আটকাতে চেক
...
document.addEventListener("DOMContentLoaded", async () => {
  await loadPopupHTML();        // ✅ Popup layout লোড
  injectPopupCSS();             // ✅ CSS যুক্ত

  // DOM-এ popup এলেমেন্ট লোড না হলে, ১ সেকেন্ড পরে আবার চেষ্টা করুন
  function waitAndLoadNotificationData(attempt = 0) {
    ...
    if (popup) {
      ...
      console.error("❌ Popup container (adminNotification) DOM-এ পাওয়া যায়নি!");
    }
  }
...
async function loadNotificationData() {
  try {
    const popup = document.getElementById("adminNotification");
    if (!popup) {
      console.warn("⚠️ Popup container (adminNotification) পাওয়া যায়নি – loadPopupHTML আবার কল করুন");
      await loadPopupHTML();
      return;
    }
...
function showNotificationFromJSON(data, encodedIdentifier) {
  const popup = document.getElementById("adminNotification");
  if (!popup) {
    console.error("❌ Popup container (adminNotification) DOM-এ নেই!");
    return;
  }

  // নিচের সব এলিমেন্ট আছে কিনা চেক, না থাকলে error দেখান
  ...
  if (!img || !imgSrc || !title || !btn || !linkEl) {
    console.error("❌ Popup HTML structure mismatch!");
    return;
  }
