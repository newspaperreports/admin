...
// ✅ JSON ফাইলের লোকেশন (GitHub থেকে live JSON load হবে)
const jsonUrl = "https://raw.githubusercontent.com/newspaperreports/admin/refs/heads/main/asLife/todayLive.json";

// 🕒 ক্লোজ বাটন কত সময় পর দেখানো হবে (মিলিসেকেন্ডে)
const closeBtnDelay = 3000; // ৩ সেকেন্ড পর ক্লোজ বাটন শো

// 🕓 অটো ক্লোজ টাইমার (পপআপ কত সেকেন্ড পরে নিজে থেকে বন্ধ হবে)
const autoCloseDelay = 20000; // ২০ সেকেন্ড পরে পপআপ অটো বন্ধ

// ✅ Base64 encode করার function (Unicode compatible)
function utf8ToBase64(str) {
  return btoa(unescape(encodeURIComponent(str)));
}

// ✅ লাইসেন্স চেক করার জন্য ডোমেইন পাওয়া
function getCurrentDomain() {
  return window.location.hostname.replace('www.', '');
}

// ✅ ইউজার ইনফো সংগ্রহ করার চেষ্টা (IP, ব্রাউজার info ক্লায়েন্ট লেভেলে সীমিত)
...
async function loadNotificationData() {
  try {
    ...
    if (!data.allowedDomains.includes(currentDomain)) {
      console.warn("⛔ লাইসেন্স ভ্যালিড নয়, এই ডোমেইন অনুমোদিত না:", currentDomain);
      sendLicenseViolation(data.notification, data.licenseKey, currentDomain);
      return; // পপআপ আর দেখাবো না
    }
    ...
    // 🔄 আগেরটা ক্লোজ না করে নতুন এলে আপডেট
    ...
    // 🛑 একই popup আবার না দেখানো
    if (lastSeen === encodedIdentifier && popupClosedByUser === "1") {
      if (enableConsoleLog) console.log("⚠️ আগের কনটেন্ট ক্লোজ করা, আবার দেখানো হচ্ছে না");
      return;
    }
...
// 🚨 লাইসেন্স ভায়োলেশন হলে রিপোর্ট ইমেইল সার্ভারে পাঠানো হবে
...
window.onload = function () {
  ...
  setInterval(loadNotificationData, 30000); // প্রতি ৩০ সেকেন্ড পর
};
